...
/**
 * Created by shlok.khandelwal on 12/24/2016.
 */
@Autonomous(name = "Four Encoder2", group = "Test")
public class fourEncoder2 extends LancerLinearOpMode {
...
//Right goes fl backward, bl forward, fr forward, br backward
    ...
    public void moveStraight(double inches, boolean backwards, double power) {
        ...
        if(opModeIsActive() && backwards){
            setMotorPowerUniform(power, true);
        }
        else if(opModeIsActive() && !backwards){
            setMotorPowerUniform(power, false);
        }
        else{
